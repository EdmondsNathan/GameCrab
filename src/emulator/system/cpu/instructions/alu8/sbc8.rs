...
impl Console {
    pub(super) fn sbc8(&mut self, arg: A8Args) -> Option<u64> {
        match arg {
            A8Args::HL => {
                self.push_command(
                    3,
                ...
                self.push_command(
                    ...
                    Update(|console: &mut Console| {
                        ...
                        let (result, carry1) = a_register.overflowing_sub(y_register);
                        let (result, carry2) = result.overflowing_sub(carry_flag);
                        let carry = carry1 || carry2;
                        let half_carry = (a_register & 0x0F) < ((y_register & 0xF) + carry_flag);
            ...
            _ => {
                self.push_command(
                    ...
                    Update(|console: &mut Console| {
                        ...
                        let a_register = console.cpu.get_register(&Register8::A);
                        let carry_flag = console.cpu.get_flag(&Flags::C) as u8;
                        let (result, carry1) = a_register.overflowing_sub(sub_register);
                        let (result, carry2) = result.overflowing_sub(carry_flag);
                        let carry = carry1 || carry2;
                        let half_carry = (a_register & 0x0F) < ((sub_register & 0xF) + carry_flag);
...
mod tests {
    ...
    #[test]
    fn sbc8_hl() {
        let mut console = init(vec![(0x9E, 0x100), (0x01, 0x200)]);
        ...
        assert_eq!(console.cpu.get_register(&Register8::A), 0x0E);
        assert!(console.cpu.get_flag(&Flags::H));
    }

    #[test]
    fn sbc8_u8() {
        let mut console = init(vec![(0xDE, 0x100), (0x01, 0x101)]);
        console.cpu.set_register(0x10, &Register8::A);
        console.cpu.set_flag(true, &Flags::C);
    ...
    #[test]
    fn sbc8_register8() {
        let mut console = init(vec![(0x98, 0x100)]);
        console.cpu.set_register(0x10, &Register8::A);

...
impl Console {
    pub(crate) fn command_ram_to_register(&mut self, address: u16, register: Register8) {
        let value = self.ram.fetch(address);

        self.cpu.set_register(value, register);
    }

    pub(crate) fn command_register_to_ram(&mut self, address: u16, register: Register8) {
